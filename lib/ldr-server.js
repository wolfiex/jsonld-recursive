...
async function handleRequest(req, res) {
  ...
  try {
    ...
    // Test URL resolution/mapping
    if (url.pathname === '/resolve' && req.method === 'POST') {
      const body = await parseBody(req);
      if (!body.url) { sendJson(res, 400, { error: 'Missing url' }); return; }

      const originalUrl = body.url;
      const resolvedUrl = applyMappings(originalUrl);
      const isLocal = !resolvedUrl.startsWith('http://') && !resolvedUrl.startsWith('https://');
      ...
      if (isLocal) {
        ...
        try {
          const result = await new Promise((resolve, reject) => {
            const client = resolvedUrl.startsWith('https:') ? https : http;
            const req = client.get(resolvedUrl, { headers: { 'Accept': 'application/ld+json, application/json' } }, (res) => {
      ...
      sendJson(res, 200, {
        ...
        changed: originalUrl !== resolvedUrl,
        isLocal: isLocal,
        exists: exists,
        fileInfo: fileInfo,
    ...
    if (url.pathname === '/test-load' && req.method === 'POST') {
      const body = await parseBody(req);
      if (!body.url) { sendJson(res, 400, { error: 'Missing url' }); return; }

      try {
        const result = await documentLoader(body.url);
        sendJson(res, 200, {
          success: true,
          documentUrl: result.documentUrl,
          hasContext: '@context' in (result.document || {}),
          keys: Object.keys(result.document || {}),
          preview: JSON.stringify(result.document).substring(0, 500) + '...'
        });
      } catch (e) {
        sendJson(res, 200, {
          success: false,
